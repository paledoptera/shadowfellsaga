...
mt_gml_token=new gml_std_haxe_enum(-1,"gml_token",["header","macro","hash","semico","comma","period","colon","qmark","at_sign","dollar_sign","keyword","ident","undefined_hx","number","cstring","un_op","adjfix","bin_op","set_op","par_open","par_close","sqb_open","sqb_close","cub_open","cub_close","arg_const","null_co","null_co_set","pragma"]);
...
function compile_groups_gml_compile_group_literal_proc(l_q,l_actions,l_out){
	...
	switch(l__g.__enumIndex__){
		...
		case 1:if(l_out)ds_list_add(l_actions,gml_action_number(l__g.h_d,l__g.h_value));break;
...
function compile_gml_compile_args_get_simple(l_q){
	...
	switch(l__g.__enumIndex__){
		case 48:
			...
			break;
		case 42:
			compile_gml_compile_args_get_simple_val=gml_compile_curr_script.h_local_map.h_obj[$ l__g.h_id];
			...
			case 2:
				...
				if(l_val==undefined){
					if(!variable_struct_exists(l_self1,l_field)){
						if(gml_std_gml_NativeTypeHelper_isNumber(l_self1)){
...
function compile_gml_compile_call_func_proc(l_actions,l_d,l_fn,l_args1,l_out){
	...
	if(l_funcFlags==0&&l_argc>0&&l_argc<=5){
		...
		if(l__g.__enumIndex__==42){
			l_firstArgVar=gml_compile_curr_script.h_local_map.h_obj[$ l__g.h_id];
			l_startArg++;
		}
	}
...
function compile_gml_compile_adjfix_proc(l_d,l_x,l_isPrefix,l_inc,l_actions,l_out){
	...
	switch(l__g.__enumIndex__){
		case 42:
			var l_localInd=gml_compile_curr_script.h_local_map.h_obj[$ l__g.h_id];
			if(l_isPrefix){
				if(l_inc){
		...
		case 48:
			...
			break;
		case 63:
			if(compile_gml_compile_args_proc(l_actions,[l__g.h_x,l__g.h_id]))return true;
		...
		case 69:
			...
			break;
		case 66:
			if(compile_gml_compile_args_proc(l_actions,[l__g.h_x,l__g.h_i1,l__g.h_i2]))return true;
		...
		case 75:
			if(compile_gml_compile_args_proc(l_actions,[l__g.h_lx,l__g.h_id]))return true;
		...
		case 81:
			if(compile_gml_compile_args_proc(l_actions,[l__g.h_lx,l__g.h_i1,l__g.h_i2]))return true;
...
function gml_func()constructor{
	...
	static h_set=function(l_sig,l_func){
		...
		if(l_argText==""){
			...
			l_argCheck=array_create(l_argCount,0);
		}
		var l_pos;
		var l_typeMap=gml_type_check_map;
		var l_i=0;
		for(var l__g1=l_argCount;l_i<l__g1;l_i++){
			...
			if(l_pos>=0){
				...
				l_arg=gml_std_string_substring(l_arg,0,l_pos);
				var l_typeFunc=l_typeMap.h_obj[$ l_typeStr];
				if(l_typeFunc!=undefined){
					if(l_isRest)l_restCheck=l_typeFunc;
					l_argCheck[@l_i]=l_typeFunc;
				} else show_error("\""+l_typeStr+"\" is not a known type (in argument["+string(l_i)+"] \""+l_arg+"\" in \""+l_sig+"\")",true);
		...
		if(is_method(l_func)){
			...
			if(l__self!=undefined){
				if(instanceof(l__self)=="instance"||gml_std_gml_NativeTypeHelper_isNumber(l__self)){
		...
		ds_map_set(gml_func_name,l_rawFunc,l_name);
		self.h_name=l_name;
		self.h_func=l_func;
...
function compile_gml_compile_bin_op_proc(l_actions,l_d,l_o,l_a,l_b){
	if(gml_op_is_simple(l_o)){
		var l_ai;
		var l__g=l_a;
		if(l__g.__enumIndex__==42)l_ai=gml_compile_curr_script.h_local_map.h_obj[$ l__g.h_id]; else l_ai=-1;
		var l_bi;
		var l__g=l_b;
		if(l__g.__enumIndex__==42)l_bi=gml_compile_curr_script.h_local_map.h_obj[$ l__g.h_id]; else l_bi=-1;
		if(l_ai>=0){
			if(l_bi>=0){
...
function gml_value_get_type(l_this1){
	if(l_this1==undefined){
		return "undefined";
	} else if(gml_std_gml_NativeTypeHelper_isNumber(l_this1)){
		return "number";
...
function vm_value_gml_value_printer_print_rec(l_v,l_z){
	...
	if(gml_std_gml_NativeTypeHelper_isNumber(l_v)){
...
if(live_enabled)
function gml_std_gml_NativeTypeHelper_isNumber(l_v){
...
function compile_gml_compile_delete_proc(l_actions,l_d,l_x){
	...
	switch(l__g.__enumIndex__){
		case 42:
			...
			break;
		case 45:ds_list_add(l_actions,vm_v2_gml_action_closure_bind(l_d,"delete global",{varname:l__g.h_id},compile_gml_compile_delete_proc_lf1));break;
		case 48:
			var l__g1=l__g.h_x;
			if(l__g1.__enumIndex__==13){
				if(gml_compile_node(l_x,l_actions,true))return true;
				ds_list_add(l_actions,vm_v2_gml_action_closure_bind(l_d,"delete self.field",{varname:l__g.h_fd},compile_gml_compile_delete_proc_lf2));
			} else {
				if(gml_compile_node(l__g1,l_actions,true))return true;
				ds_list_add(l_actions,vm_v2_gml_action_closure_bind(l_d,"delete field",{varname:l__g.h_fd},compile_gml_compile_delete_proc_lf3));
			}
			break;
		case 63:
			if(compile_gml_compile_args_proc(l_actions,[l__g.h_x,l__g.h_id]))return true;
...
function compile_groups_gml_compile_group_ds_proc(l_q,l_actions,l_out){
	...
	switch(l__g.__enumIndex__){
		case 75:
			...
			if(l_fn!=undefined)return compile_gml_compile_call_func_proc(l_actions,l__g.h_d,l_fn,[l__g.h_lx,l__g.h_id],l_out); else return gml_compile_error("Accessor not supported",l__g.h_d);
		case 76:
			l_fn=gml_func_map.h_obj[$"ds_list_set"];
			if(l_fn!=undefined)return compile_gml_compile_call_func_proc(l_actions,l__g.h_d,l_fn,[l__g.h_lx,l__g.h_id,l__g.h_v],l_out); else return gml_compile_error("Accessor not supported",l__g.h_d);
		case 77:
			var l_args1=[l__g.h_lx,l__g.h_id,l__g.h_v];
			l_fn=gml_func_map.h_obj[$"ds_list_find_value"];
			l_fn2=gml_func_map.h_obj[$"ds_list_set"];
			if(l_fn!=undefined&&l_fn2!=undefined){
				compile_gml_compile_args_proc(l_actions,l_args1);
				ds_list_add(l_actions,gml_action_ds_aop(l__g.h_d,l__g.h_o,array_length(l_args1),l_fn.h_func,l_fn2.h_func,l_out));
				return false;
		...
		case 84:
			...
			if(l_fn!=undefined)return compile_gml_compile_call_func_proc(l_actions,l__g.h_d,l_fn,[l__g.h_x,l__g.h_id],l_out); else return gml_compile_error("Accessor not supported",l__g.h_d);
		case 85:
			l_fn=gml_func_map.h_obj[$"variable_struct_set"];
			if(l_fn!=undefined)return compile_gml_compile_call_func_proc(l_actions,l__g.h_d,l_fn,[l__g.h_x,l__g.h_id,l__g.h_v],l_out); else return gml_compile_error("Accessor not supported",l__g.h_d);
		case 86:
			var l_args1=[l__g.h_x,l__g.h_id,l__g.h_v];
			l_fn=gml_func_map.h_obj[$"variable_struct_get"];
		...
		case 81:
			...
			if(l_fn!=undefined)return compile_gml_compile_call_func_proc(l_actions,l__g.h_d,l_fn,[l__g.h_lx,l__g.h_i1,l__g.h_i2],l_out); else return gml_compile_error("Accessor not supported",l__g.h_d);
		case 82:
			l_fn=gml_func_map.h_obj[$"ds_grid_set"];
			if(l_fn!=undefined)return compile_gml_compile_call_func_proc(l_actions,l__g.h_d,l_fn,[l__g.h_lx,l__g.h_i1,l__g.h_i2,l__g.h_v],l_out); else return gml_compile_error("Accessor not supported",l__g.h_d);
		case 83:
			var l_args1=[l__g.h_lx,l__g.h_i1,l__g.h_i2,l__g.h_v];
			l_fn=gml_func_map.h_obj[$"ds_grid_get"];
...
function compile_groups_gml_compile_group_array_proc(l_q,l_actions,l_out){
	...
	switch(l__g.__enumIndex__){
		case 63:
			...
			var l_i;
			var l__g1=l_arr;
			if(l__g1.__enumIndex__==42)l_i=gml_compile_curr_script.h_local_map.h_obj[$ l__g1.h_id]; else l_i=-1;
			if(l_i>=0){
		...
		case 72:
			...
			break;
		case 67:
			if(compile_gml_compile_args_proc(l_actions,[l__g.h_x,l__g.h_i1,l__g.h_i2,l__g.h_v]))return true;
		...
		case 68:
			if(compile_gml_compile_args_proc(l_actions,[l__g.h_x,l__g.h_i1,l__g.h_i2,l__g.h_v]))return true;
			ds_list_add(l_actions,gml_action_index2d_aop(l__g.h_d,l__g.h_o));
			break;
		case 74:
			if(compile_gml_compile_args_proc(l_actions,[l__g.h_x,l__g.h_i1,l__g.h_i2,l__g.h_v]))return true;
			ds_list_add(l_actions,gml_action_index2d_aop(l__g.h_d,l__g.h_o));
	...
	if(l__g.__enumIndex__==22){
		...
		switch(l__g.__enumIndex__){
			case 16:
				...
				break;
			case 48:gml_std_haxe_enum_tools_setTo(l_q,gml_node_call_field(l_d,l__g.h_x,l__g.h_fd,l_args1));break;
			case 12:
				var l_s1=l__g.h_id;
...
function gml_seek_fields_proc(l_q,l_st){
	...
	if(l__g.__enumIndex__==48){
		...
		var l_x=l__g.h_x;
		var l_s=l__g.h_fd;
		if(l_x.__enumIndex__==15)gml_std_haxe_enum_tools_setTo(l_q,gml_node_global_hx(l_d,l_s));
	}
	return gml_node_tools_seek(l_q,l_st,gml_program_seek_func);
}
...
function gml_std_gml_internal_ArrayImpl_slice(l_arr,l_pos,l_end){
	...
	var l_out=array_create(l_len,0);
	...
	while(++l_tokensPos<l_tokensLen){
		...
		if(l_src!=undefined){
			if(l_tokensPos>0){
				...
				if((l__g1.__enumIndex__==5))continue;
			}
			if(l_tokensPos<l_tokensLen1){
				var l__g3=l_tokens[l_tokensPos+1];
				if((l__g3.__enumIndex__==5))live_gmlive_patcher_add_source(l_id,l_acc,l_found,l_src);
			}
			continue;
		}
...
function gml_action_list_print_action_value(l_v){
	if(l_v==undefined){
		return "null";
	} else if(gml_std_gml_NativeTypeHelper_isNumber(l_v)){
...
function gml_action_list_print_action_get_func_name(l_func){
	...
	} else if(gml_std_gml_NativeTypeHelper_isNumber(l_func)&&script_exists(l_func)){
...
function gml_type_check_number(l_v){
	if(gml_std_gml_NativeTypeHelper_isNumber(l_v))return "";
...
function gml_type_check_z_number(l_v){
	if(l_v==undefined||gml_std_gml_NativeTypeHelper_isNumber(l_v))return "";
	if(is_string(l_v))return "";
...
function vm__gml_with_data_gml_with_data_impl__alloc(l_size){
	...
	if(ds_stack_empty(l_pool))return array_create((power(10,l_sln)|0),0); else return ds_stack_pop(l_pool);
}
...
if(live_enabled)
function vm_v2_gml_thread_group_func_literal_create_function_lf(){
...
function vm_v2_gml_thread_group_func_literal_create_function(l_program,l__name,l__self){
	...
	return method(l_meta,vm_v2_gml_thread_group_func_literal_create_function_lf);
}
