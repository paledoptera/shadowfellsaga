...
if (message_end > 0)
{
	...
	// Portraits cannot be defined for bubbles so we simply set the variable
	var tX = textX;
    ...
    {   
		...
		{
			if string_char_at(message[message_current], i-1) = char_small[s] {textX -= char_small_value} 
		}
		s = 0;
		for(s=0; s<array_length(char_big); s++;)
		{
			if string_char_at(message[message_current], i-1) = char_big[s] {textX += char_big_value} 
		}
        ...
        switch(modifier)
        {
            case 0: //normal
            {
				...
				if(flag_get(global.flags,"world")==0){
					 draw_text_transformed(tX+(space*charWidth), tY+(26*line)+(line*charLineSep), string_hash_to_newline(string_char_at(message[message_current], i)),textScale,textScale,0);
				} else if(flag_get(global.flags,"world")==1){
					 draw_text_transformed_shadow(tX+(space*charWidth), tY+(26*line)+(line*charLineSep), string_hash_to_newline(string_char_at(message[message_current], i)),textScale,textScale,0,c_white,1,font);
				}
                break;
            }
            case 1: //shaky
            {
                draw_set_colour(c_white);
				if(flag_get(global.flags,"world")==0){
					draw_text(textX+(space*charWidth)+round(random_range(-2, 2)), tY+(26*line)+(line*charLineSep)+round(random_range(-2, 2)), string_hash_to_newline(string_char_at(message[message_current], i)));
				} else if(flag_get(global.flags,"world")==1){
					draw_text_transformed_shadow(textX+(space*charWidth)+round(random_range(-2, 2)), tY+(26*line)+(line*charLineSep)+round(random_range(-2, 2)), string_hash_to_newline(string_char_at(message[message_current], i)),textScale,textScale,0,c_white,1,font);
				}
                break;
            }
            case 2: //color
            {
				...
				if(flag_get(global.flags,"world")==0){
					 draw_text(textX+(space*charWidth), textY+(26*line)+(line*charLineSep), string_hash_to_newline(string_char_at(message[message_current], i)));
				} else if(flag_get(global.flags,"world")==1){
					draw_text_transformed_shadow(textX+(space*charWidth), texY+(26*line)+(line*charLineSep), string_hash_to_newline(string_char_at(message[message_current], i)),textScale,textScale,0,trueColor,1,font);
				}
                break;
            }
            case 3: //color and shake
            {
				...
				if(flag_get(global.flags,"world")==0){
					...
					draw_text_transformed_shadow(tX+(space*charWidth)+round(random_range(-2, 2)), tY+(26*line)+(line*charLineSep)+round(random_range(-2, 2)), string_hash_to_newline(string_char_at(message[message_current], i)),textScale,textScale,0,c_red,1,font);
				}
                break;
            }
            case 4: //Sine movement
            {
				...
				if(flag_get(global.flags,"world")==0){
					 ...
					 draw_text_transformed_shadow(tX+(space*charWidth), tY+(26*line)+shift+(line*charLineSep), string_hash_to_newline(string_char_at(message[message_current], i)),textScale,textScale,0,c_white,1,font);
				}
                break;
            }
            case 5: //Gradient Text
            {
				...
				if(flag_get(global.flags,"world")==0){
					...
					draw_text_transformed_shadow(tX+(space*charWidth), tY+(26*line)+(line*charLineSep), string_hash_to_newline(string_char_at(message[message_current], i)),textScale,textScale,0,gradientcol,1,font);
				}
                break;
            }
            case 6: //Gradient & Sine
            {
				...
				if(flag_get(global.flags, "world")==0){
					 ...
					 draw_text_transformed_shadow(tX+(space*charWidth), tY+(26*line)+shift, string_hash_to_newline(string_char_at(message[message_current], i)),textScale,textScale,0,gradientcol,1,font);
				}
                break;
            }
        }
