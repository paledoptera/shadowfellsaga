	...
	case 1:
			...
			switch(subSelection){
				case 0:
					...
					selectedItem=clamp(selectedItem,0,10);
											var valid_ids  = [1]
					switch(selectedItem){
						case 0:
							optSoulSX=90;
						...
						case 5:
							...
							if  pressed("right")
								selectedItem=11;
								item_exists_inslot(selectedItem,"dcandy")
						break;
						case 6:
							optSoulSX=190+150;
							optSoulSY=175;
		...
		switch configHubSelection
		{
			...
			case 0:
				...
				if(pressed("action")&&cmenuActive==true){
						...
						cmenuState=(3<<1);
						cmenuActive=true;
						var _master = instance_create_depth(420,152,-601,obj_slider_volume)
						with(_master){ type = "master" };
						
						var _music = instance_create_depth(420,192,-601,obj_slider_volume)
						with(_music){ type = "music" };
						
						var _sfx = instance_create_depth(420,232,-601,obj_slider_volume)
						with(_sfx){ type = "sfx" };
						
						var _voice = instance_create_depth(420,272,-601,obj_slider_volume)
...
if cmenuState == (3<<2){
	...
	if(!input_binding_scan_in_progress()){
		subSelection = clamp(subSelection, 0, 12);
		var notRebinding = false;
		if subSelection < 10 
		{
			if  pressed("down")
				subSelection++
			...
			if(subSelection == 11)&&pressed("action"){
				subState = 0;
				input_player_export();
				cmenuState = (0<<0);
			}
		}
		if !input_binding_scan_in_progress()&&check("action"){
			switch(subSelection)
			{
			...
			// Esnure we don't scan for vk_escape, the function keys or backspace (as backspace is used to cancel rebinding)
			input_binding_scan_params_set([vk_backspace,vk_f1,vk_f2,vk_f3,vk_f4,vk_f5,vk_f6,vk_f7,vk_f8,vk_f9,vk_f10,vk_f11,vk_f12,vk_escape]);
...
if cmenuState == (3<<3){
	...
	if _right {
		res_sel = number_add(res_sel,12,1);
	...
	if subSelection == 2 {
		if  pressed("right")
			...
			if (global.camtype == 1){cam_label = "Smooth" };
			if (global.camtype == 0){cam_label = "Rigid" };
		global.camtype = clamp(global.camtype,0,1);
	}
	#endregion
